...
visitToggle.addEventListener('click', async () => {
  ...
  try {
    ...
    const response = await fetch(
      'https://m12-test-render-backend.onrender.com/backend/public/index.php?action=actualizarEstadoVisita', 
      {
        method: 'POST',
        headers: { 
...
async function getVisited() {
  if (session.logged) {
      try {
          const response = await fetch("https://m12-test-render-backend.onrender.com/backend/public/index.php?action=obtenerVisitados", {
              method: 'GET',
              credentials: 'include'
          });
  ...
  document.querySelectorAll('.emoji-rating').forEach(container => {
    ...
    emojis.forEach(emoji => {
      emoji.addEventListener('click', () => {
        ...
        fetch("https://m12-test-render-backend.onrender.com/backend/public/index.php?action=saveRating", {
          method: 'POST',
          credentials: 'include',
          headers: {
            'Content-Type': 'application/json'
...
function setupDeleteVisitedListeners() {
  document.querySelector('.fav-content').addEventListener('click', async (event) => {
    if (event.target.classList.contains('delete-visitado')) {
      ...
      try {
        const response = await fetch('https://m12-test-render-backend.onrender.com/backend/public/index.php?action=actualizarEstadoVisita', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
